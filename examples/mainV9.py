...
def preprocess_pong(image):
    ...
    # Downsample width and height by a factor of 4
    return I.astype(np.float).reshape((40, 4, 40, 4)).max(3).max(1).reshape((40, 40, 1))
...
def prapare_pong_observation(current_frame, previous_frame):
    ...
    return combine_frames(cur, prev)

def combine_frames(current_frame, previous_frame):
  change = current_frame - previous_frame
  return np.concatenate((change, current_frame), 0)
...
if runPong:
  ...
  pong_weights_folder = 'examples\output\{}_dqn_{}\weights'.format('v9', ENV_NAME)
  ...
  for i_episode in range(MAX_ITERS):
    ...
    while True:
        ...
        obs_change = combine_frames(current_frame, previous_frame)
        # obs_change = # TODO
